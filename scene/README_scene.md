...
/ABB/tip (tip) ↔ /ABB/target (target) avec le type **IK, tip-target**
...
**Pour déplacer le robot depuis Python :** il faut uniquement déplacer /ABB/target. Ne jamais définir directement les positions des articulations.
...
Ouvrir `smartfactory_scene.ttt`

         ↓
Lancer la simulation (`sim.startSimulation()`)

         ↓
Le module de trajectoire IA génère les positions cibles

         ↓
Python déplace `/ABB/target` pas à pas

         ↓
Le script IK déplace automatiquement les articulations du robot

         ↓
Le module qualité lit la position de `/ABB/tip` et dessine le cordon

         ↓
Le rapport de fin de cycle est généré
